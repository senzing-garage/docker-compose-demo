...
1. Make directory for certifications.
   Example:

    ```console
    mkdir ${SENZING_ETC_DIR}/ssh
    ```

#### Generate self-signed Certificate Authority

1. :pencil2: Create a password for the certificates.
   Example:
...
1. Restrict visibility of the key file.
   Example:
    ...
    ```console
    chmod 400 ${SENZING_ETC_DIR}/ssh/ca-key.pem
    ```
...
1. :thinking: **Optional:**
   ...
   Example:
    ...
    ```console
    openssl x509 \
      -in ${SENZING_ETC_DIR}/ssh/ca-cert.pem \
      -noout \
      -text
    ```

#### Generate Private Key and Certificate Signing Request for Kafka
...
1. Create `kafka-cert.pem` using
   [openssl](https://www.openssl.org/docs/man1.1.1/man1/openssl-req.html).
   When prompted for "Enter pass phrase for / ... /ca-key.pem:",
   enter the value of `OPENSSL_CA_PASS_PHRASE`.
   Example:

    ```console
    openssl x509 \
      -CA ${SENZING_ETC_DIR}/ssh/ca-cert.pem \
      -CAcreateserial \
      -CAkey ${SENZING_ETC_DIR}/ssh/ca-key.pem \
      -days 180 \
      -in ${SENZING_ETC_DIR}/ssh/kafka-req.pem \
      -out ${SENZING_ETC_DIR}/ssh/kafka-cert.pem \
      -req
    ```
